...
public:
          ...
          visualizer(nh)
    {
        ...
        corridor_generator = std::make_shared<CorridorGen::CorridorGenerator>(config.resolution, config.clearance, config.max_sample, config.ceiling, config.floor_limit, config.goal_pt_margin, true, no_flight_zone);
    ...
    inline void plan()
    {
        if (startGoal.size() == 2)
        {
            ...
            bool generate_success = corridor_generator->generateCorridorAlongPath(route);
            std::cout << "generate success: " << generate_success << std::endl;
            std::vector<Corridor> corridor_list = corridor_generator->getCorridor();
            std::vector<Eigen::Vector3d> initial_guess = corridor_generator->getWaypointList();
